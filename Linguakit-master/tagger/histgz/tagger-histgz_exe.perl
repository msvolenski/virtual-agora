...
my $MODEL;#<file>
open ($MODEL, $abs_path."/model/train-histgz") or die "O ficheiro train-gl não pode ser aberto: $!\n";
binmode $MODEL,  ':utf8';#<ignore-line>

##variabeis globais
my $w=1; #<string>#mesma janela/window que no treino
my $Border = "(Fp|<blank>)";#<string>
my @cat_open = ("NC", "VMI", "VMS", "VMG", "VMP", "VMN", "AQ");#<array><string>
...
sub tagger {
	...
	my $tag;#<string>
	my @noamb=();#<array><@text><boolean>
	my @unk=();#<array><@text><boolean>
	my @Token=();#<array><@text><string>
	my @Tag=();#<array><@text><string>
	my @Lema=();#<array><@text><string>
	...
	foreach my $line (@{$text}) {
		...
		if ($entry[2] !~ /^$Border$/) {
			...
			if  ($unk[$pos] && $Token[$pos] =~ /[\w]+mente$/) { ##se a forma e desconhecida acabada em -mente, RG
				...
				$Lema{$pos}{$Tag{$pos}{$cat}} = $Lema[$pos] ;
				$unk[$pos]=0;
			}elsif  ($unk[$pos] &&  $Token[$pos] !~ /[áéíóú]/ && $Token[$pos] =~ /[\w]+ría([sn])?$/) { ##se a forma e desconhecida acabada em -ría, VMI
				my  $cat = "VMI";#<string>
				$Tag{$pos}{$cat} = "VMIC000";
				$Lema{$pos}{$Tag{$pos}{$cat}} = $Lema[$pos] ;
				$unk[$pos]=0;
					...
					if ($pos==0) {
						for (my $j=1;$j<=$w;$j++) {#<integer>
							...
							foreach my $feat (keys %{$Tag{$j}}) {
								...
								my $featL = $feat . "_" .  $Token[$pos];#<string>
...
sub rules_neg {    #regras lexico-sintacticas negativas
	...
	#impedir artigo final de frase ou seguido de preposiçao ou de conjunção ou simbolo
	elsif ($cat =~ /^D/ && ($feat eq "R_END" || $feat eq "R_SP" || $feat eq "R_CC" || $feat eq "R_F") ) {
		$result = 1;
	}
	...
	#elsif ($cat =~ /^PP/ && ($FEAT !~ /L_V[A-Z]/ && $FEAT !~ /R_V[A-Z]/) ) {
	#	print STDERR "pron: #$cat# -- #$FEAT#\n";
	#	$result = 1;
	#}
	return $result;
...
sub rules_pos {    #regras lexico-sintacticas positivas
	...
	##se aparece a forma 'nos' diante de nome comum e sp-det
	elsif ($cat =~ /^SP/   && $feat =~ /(R_NC_nos|R_DP_nos)/  ) {
		$result = 1;
	}
